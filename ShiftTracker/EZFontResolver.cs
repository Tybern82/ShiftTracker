...
namespace EZFontResolverNS {
    ...
    public class EZFontResolver : IFontResolver {
        ...
        /// </exception>
        public void AddFont(string familyName, XFontStyleEx style, string filename,
        ...
        /// <param name="simulateBold">if set to <c>true</c> bold will be simulated.</param>
        /// <param name="simulateItalic">if set to <c>true</c> italic will be simulated.</param>
        public void AddFont(string familyName, XFontStyleEx style, byte[] data,
            bool simulateBold = false, bool simulateItalic = false) {
            ...
            if (simulateBold && (style & XFontStyleEx.Bold) == 0) {
                AddFontHelper(familyName, style | XFontStyleEx.Bold, data, true, false);
            }

            // Same for italic.
            if (simulateItalic && (style & XFontStyleEx.Italic) == 0) {
                AddFontHelper(familyName, style | XFontStyleEx.Italic, data, false, true);
            }

            // Same for bold and italic.
            if (simulateBold && (style & XFontStyleEx.Bold) == 0 &&
                simulateItalic && (style & XFontStyleEx.Italic) == 0) {
                AddFontHelper(familyName, style | XFontStyleEx.BoldItalic, data, true, true);
            }
        }

        void AddFontHelper(string familyName, XFontStyleEx style, byte[] data, bool simulateBold, bool simulateItalic) {
